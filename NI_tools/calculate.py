...
def compute_spectrogram(x, calibration, npts=512, noverlap=256, window='hann', average='mean', compute=True):
    ...
    template = xr.DataArray(np.ones((hours, int(
        npts/2+1))), dims=['time', 'frequency']).chunk({'time': 1, 'frequency': int(npts/2+1)})

    spectrogram = x.map_blocks(processing.psd_estimate_chunk, args=[
                               calibration, npts, noverlap, window, average], template=template)
...
def compute_NCCF_stack(ds, W=30, Fs=200, compute=True, stack=True):
    ...
    NCCF_stack = ds.map_blocks(
        processing.NCCF_chunk, template=da_temp, args=[stack])
